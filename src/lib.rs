...
impl App {
    ...
    async fn ensure_render_state_for_surface(&mut self) {
        ...
        if self.render_state.is_none() {
            ...
            let swapchain_format = surface_state.surface.get_capabilities(adapter).formats[0];
    ...
    fn configure_surface_swapchain(&mut self) {
        let (Some(render_state), Some(surface_state)) = (&self.render_state, &self.surface_state) else {
            return;
        };
        ...
        log::info!(
            "WGPU: Configuring surface swapchain: format = {swapchain_format:?}, size = {size:?}"
        );
        surface_state
            .surface
    ...
    event_loop.run(move |event, event_loop, control_flow| {
        ...
        *control_flow = ControlFlow::Wait;
        match event {
            ...
            Event::RedrawRequested(_) => 'redraw_requested: {
                log::info!("Handling Redraw Request");

                let Some(ref surface_state) = app.surface_state else {
                    break 'redraw_requested;
                };
                let Some(ref rs) = app.render_state else {
                    break 'redraw_requested;
                };
